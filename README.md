...
This repo bundles a `Dockerfile` (Hermes base image + the system/Python tools its skills need), a Windows-friendly single-container `docker-compose.windows.yml` (s6-supervised gateway + dashboard), and ready-to-use agent configs.
...
4. **Run Hermes' setup wizard.** Hermes ships an interactive wizard (`hermes setup`) that writes `config.yaml` for you — don't hand-author it. Run it through the `hermes` service so it writes into the mounted `.hermes` volume (the compose file mounts `./.hermes` → `/opt/data`, which is where the container reads its config). Run this **before** starting the stack (step 6) so the one-off wizard container doesn't contend with the running gateway for the data volume:
   ...
   ```bash
   docker compose -f docker-compose.windows.yml run --rm -it hermes setup
   ```
...
| `docker-compose.windows.yml` | Windows Docker Desktop compose (explicit port maps instead of `network_mode: host`). Defines a single `hermes` service; the image's s6-overlay supervises the gateway + dashboard inside it, with the dashboard toggled on via `HERMES_DASHBOARD=1`. |
...
This is wired up in `docker-compose.windows.yml`:
...
The image ships the [Claude Code](https://claude.com/claude-code) CLI (installed in the `Dockerfile`), so you can run `claude` inside the container — e.g. from the dashboard's embedded chat shell or via `docker compose -f docker-compose.windows.yml exec hermes claude`.
...
The fastest way to create a valid config is Hermes' own setup wizard — `docker compose -f docker-compose.windows.yml run --rm -it hermes setup` ([Quick start](#quick-start) step 4). It writes `.hermes/config.yaml` for you; the blocks below are what to set/refine afterward.
...
**Any other secret env var** belongs in `.env` too (extra keys, tokens, local overrides). To pass one into the container, also reference it in `docker-compose.windows.yml` under the `hermes` service's `environment:` (e.g. `- MY_VAR=${MY_VAR:-}`). Both `.env` and `.hermes/.env` are git-ignored (see [`.gitignore`](./.gitignore)); keep it that way even in a private fork.
